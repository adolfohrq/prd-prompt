...
import { TrashIcon, EyeIcon, GeneratePromptIcon } from '../components/icons/Icons';
...
export const MyDocuments: React.FC<MyDocumentsProps> = ({ prds, prompts, onDeletePrd, onDeletePrompt, onViewDocument }) => {
  ...
  return (
    <div>
      ...
      <section>
        <h2 className="text-2xl font-semibold text-gray-800 mb-4">Documentos de Requisitos (PRDs)</h2>
        {prds.length === 0 ? (
          <p className="text-gray-500">Nenhum PRD criado ainda.</p>
        ) : (
            ...
            {prds.map(prd => (
              <Card key={prd.id} className="flex flex-col">
                <div className="flex-grow">
                  ...
                  <p className="text-sm text-gray-500 mt-1">Criado em: {formatDate(prd.createdAt)}</p>
                  <p className="text-sm text-gray-700 mt-2 line-clamp-3">{prd.ideaDescription}</p>
                </div>
      ...
      <section className="mt-12">
        <h2 className="text-2xl font-semibold text-gray-800 mb-4">Prompts de IA</h2>
         {prompts.length === 0 ? (
            ...
            {prompts.map(prompt => (
              <Card key={prompt.id} className="flex flex-col">
                 <div className="flex-grow">
                    ...
                    <p className="text-sm text-gray-500 mt-1">Gerado em: {formatDate(prompt.createdAt)}</p>
